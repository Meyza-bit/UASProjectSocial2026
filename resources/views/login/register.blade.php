...
@section('title', 'Daftar Akun Baru — Mari Berbagi')

@section('content')
<div class="min-h-[80vh] flex flex-col justify-center items-center px-4 py-8 bg-slate-50">
    <div class="w-full max-w-md bg-white border border-slate-200/80 rounded-3xl p-8 shadow-sm space-y-6">
        
        {{-- Header Form --}}
        <div class="text-center space-y-2">
            <span class="text-3xl">✨</span>
            <h1 class="text-xl font-extrabold text-slate-900 tracking-tight">Buat Akun Baru</h1>
            <p class="text-xs text-slate-500">Mari bergabung untuk mulai berbagi kebaikan di Pontianak.</p>
        </div>
        ...
        {{-- Form Register --}}
        <form action="{{ route('register') }}" method="POST" class="space-y-4 text-xs font-medium">
            @csrf
            
            {{-- Input Nama Lengkap --}}
            <div class="space-y-1.5">
                <label for="name" class="text-slate-600 font-bold">Nama Lengkap</label>
            ...
            <div class="space-y-1.5">
                <label for="password_confirmation" class="text-slate-600 font-bold">Konfirmasi Kata Sandi</label>
        ...
        <div class="text-center text-[11px] text-slate-500 font-medium pt-2 border-t border-slate-100">
            Sudah punya akun? <a href="{{ route('login') }}" class="text-emerald-800 font-bold hover:underline">Masuk disini</a>
        </div>
